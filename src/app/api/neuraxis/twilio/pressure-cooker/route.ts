...
const CONFIG_PATH = "hive/current/kaironull_pressure_cooker_phone_workroom.yaml";
...
async function fetchHiveFile(path: string): Promise<string> {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent": "NULLWORKS-Pressure-Cooker-Phone-Gate",
    },
    cache: "no-store",
  });
...
function pinPrompt(request: Request, attempt: number, prefix = ""): Response {
  ...
  const action = `${origin}/api/neuraxis/twilio/pressure-cooker?phase=verify&attempt=${attempt}`;
  const message = `${prefix} Enter the four digit Pressure Cooker passcode, then press pound.`.trim();
...
async function handle(request: Request): Promise<Response> {
  ...
  if (phase !== "verify") {
    return pinPrompt(request, 1, "NULLWORKS Pressure Cooker Workroom.");
  }
  ...
  try {
    ...
    if (digits.length === 4 && safeEqualHex(digits, expectedHash)) {
      const target = `${url.origin}/api/neuraxis/twilio/voice?room=pressure`;
      return twiml(`<?xml version="1.0" encoding="UTF-8"?><Response>${speak("Passcode accepted. Opening the NULLWORKS Pressure Cooker Workroom.", request.url)}<Redirect method="POST">${xmlEscape(target)}</Redirect></Response>`);
    }
    ...
    console.error("Pressure Cooker gate failed", error);
    return twiml(`<?xml version="1.0" encoding="UTF-8"?><Response>${speak("The Pressure Cooker gate is temporarily unavailable. Please try again shortly.", request.url)}<Hangup/></Response>`, 503);
  }
}
