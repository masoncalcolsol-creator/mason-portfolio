...
export async function POST(request: Request) {
  ...
  if (/^(done|stop|goodbye|hang up|finish)$/.test(command)) {
    after(async () => {
      await appendCallTurn({
        ...
        room: "private",
        step: "gray_matter_session_closed",
        preserveSpeech: false,
        capturedFields: { lane: "GRAY_MATTER", action: "SESSION_CLOSED_BY_MASON" },
      }).catch(() => undefined);
    });
  ...
  try {
    if (/\b(daily triage|daily digest|what(?:'s| is) open|read my list|status|brief me)\b/.test(command)) {
      ...
      step = "gray_matter_digest_sent_to_self";
      ...
      step = "gray_matter_journal_search";
      ...
      if (!query) {
        ...
        capturedFields = { lane: "GRAY_MATTER", query, result_ids: results.map((result) => result.id) };
      }
    } else if (/\b(reply to|send an email|email them|respond to)\b/.test(command)) {
      step = "gray_matter_outbound_instruction_captured";
      ...
      capturedFields = { lane: "GRAY_MATTER", entry_id: result.entry.id, action_ids: result.actions.map((action) => action.id), warnings: result.warnings };
    } else if (heard) {
      const result = await captureGrayMatter({ transcript: heard, source: "TWILIO", callSid: params.CallSid });
      step = "gray_matter_journal_entry_captured";
      ...
      step = "gray_matter_empty_transcript";
    }
  } catch (error) {
    ...
    step = "gray_matter_command_failed";
    capturedFields = { lane: "GRAY_MATTER", error: error instanceof Error ? error.message : "unknown" };
  }
