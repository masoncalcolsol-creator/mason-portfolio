      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .nwIntake {
          ...
          color: #15211d;
          background:
            radial-gradient(circle at 8% 0%, rgba(46,104,112,.14), transparent 32rem),
            radial-gradient(circle at 92% 7%, rgba(189,139,53,.17), transparent 30rem),
            #f3eee2;
        ...
        .header {
          ...
          padding: 13px 15px; border: 1px solid rgba(143,104,32,.3); border-radius: 24px;
        ...
        .brandTop, .eyebrow { color: #80601f; font-size: 10px; font-weight: 950; letter-spacing: .2em; text-transform: uppercase; }
        .brandName { margin-top: 2px; color: #171914; font-family: Georgia, "Times New Roman", serif; font-size: 17px; font-weight: 800; }
        ...
        .introCopy, .boundary, form { border: 1px solid rgba(143,104,32,.3); border-radius: 32px; box-shadow: 0 28px 90px rgba(49,41,27,.11); }
        .introCopy { padding: clamp(28px,5vw,52px); background: rgba(255,251,242,.97); color: #15211d; }
        .eyebrow { display: inline-flex; padding: 8px 11px; border: 1px solid rgba(184,138,52,.34); border-radius: 999px; background: #eee1c7; }
        .title { margin: 22px 0 18px; color: #15211d !important; font-family: Georgia, "Times New Roman", serif; font-size: clamp(46px,7vw,78px); line-height: .94; letter-spacing: -.055em; }
        .lead { margin: 0; color: #51493d !important; font-size: 17px; line-height: 1.72; }
        ...
        .boundary p { margin: 18px 0 0; color: rgba(255,250,240,.76); line-height: 1.7; }
        form { display: grid; gap: 14px; margin-top: 16px; padding: clamp(24px,4vw,42px); background: rgba(255,251,242,.97); color: #15211d; }
        .pair { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        label { display: grid; gap: 8px; color: #302e29; font-size: 13px; font-weight: 900; }
        input, textarea, select {
          width: 100%; border: 1px solid rgba(103,85,52,.34); border-radius: 17px;
          background: #fffaf0; color: #15211d; padding: 15px 16px; font: inherit; outline: none;
        ...
        .hint { color: #746b5d; font-size: 12px; font-weight: 500; line-height: 1.48; }
        .scaleNote { padding: 14px 16px; border-left: 3px solid #2e6870; border-radius: 14px; background: #edf2ec; color: #4f5e59; font-size: 12px; line-height: 1.55; }
        ...
        .note { margin-top: 8px; color: #746b5d; font-size: 12px; line-height: 1.58; }
      ...
      <div className="shell">
        ...
        <section className="intro">
          <article className="introCopy">
            ...
            <h1 className="title">Show us one real workflow.</h1>
        ...
        <form onSubmit={submit}>
          <div className="pair">
            <label>Your name<input required value={form.name} onChange={(event) => updateField("name", event.target.value)} /></label>
            <label>Company<input required value={form.company} onChange={(event) => updateField("company", event.target.value)} /></label>
          </div>

          <div className="pair">
            <label>Your role<input required value={form.role} onChange={(event) => updateField("role", event.target.value)} /></label>
          ...
          <label>One real workflow<textarea required value={form.workflow} onChange={(event) => updateField("workflow", event.target.value)} /><span className="hint">Describe what starts the work, who touches it, and where it is supposed to end.</span></label>
          <label>Intended outcome<textarea required value={form.intendedOutcome} onChange={(event) => updateField("intendedOutcome", event.target.value)} /><span className="hint">Why does this workflow exist, and who depends on the result?</span></label>
          <label>Current AI and software touchpoints<textarea value={form.aiTools} onChange={(event) => updateField("aiTools", event.target.value)} /><span className="hint">List tools, copilots, agents, inboxes, spreadsheets, handoffs, or manual patches.</span></label>
          <label>One recent failure or exception<textarea required value={form.failure} onChange={(event) => updateField("failure", event.target.value)} /><span className="hint">What went wrong, what did the operator do next, and what consequence followed?</span></label>
          <label>What has already been tried?<textarea value={form.priorAttempts} onChange={(event) => updateField("priorAttempts", event.target.value)} /><span className="hint">Preserve the decision history so a failed solution is not recommended again.</span></label>
          <label>Who owns the consequence?<input required value={form.consequenceOwner} onChange={(event) => updateField("consequenceOwner", event.target.value)} /><span className="hint">Name the role or person accountable when the workflow succeeds or fails.</span></label>
