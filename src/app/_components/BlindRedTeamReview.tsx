...
export default function BlindRedTeamReview({reviewerName,firstName,artifactName,artifactVersion,artifactUrl,responseEmail,responseSubject}:BlindRedTeamReviewProps){
  ...
  const viewerUrl=`https://docs.google.com/gview?embedded=1&url=${encodeURIComponent(`https://nullworks.systems${artifactUrl}?download=1`)}`;
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.compactHero}><div className={styles.eyebrow}>Cold review</div><h1 className={styles.title}>{firstName}, try to break this.</h1><p className={styles.lead}>Your target is <strong>{artifactName}</strong> ({artifactVersion}). Read this copy before looking up NULLWORKS, Continuity Calculus, or any supporting material.</p><div className={styles.actions}><a className={styles.primary} href={viewerUrl} target="_blank" rel="noreferrer">Open paper</a><a className={styles.secondary} href={`${artifactUrl}?download=1`}>Download PDF</a><a className={styles.secondary} href={mailto}>Send first-pass review</a></div></section>
